...
class Attributes
{
   ...
   public static  function productDiscountMsg($result)
    {
        $message = '';
    if($result->discount > 0){
        if ($result->discount_type == 'flat') {
            $discount = fmod($result->discount, 1) == 0 ? (int) $result->discount : $result->discount;
            $message = '<span class="Discount">' . floor($discount) . ' Rs OFF</span>';
        } elseif ($result->discount_type == 'percentage') {
            $discount = $result->discount;
            $message = '<span class="Discount">' . floor($discount) . '% OFF</span>';
        }else{
            $message = '<span class="Discount">New</span>';
        }
    }else{
        $message = '<span class="Discount">New</span>';
